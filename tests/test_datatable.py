...
def test_datatable_data_attributes(page: Page):
    ...
    assert data_anchor is not None and data_anchor != "", "'data-anchor' attribute should exist and not be empty"
    assert data_index is not None and data_index != "", "'data-index' attribute should exist and not be empty"
...
def test_row_click_opens_side_panel(page: Page):
    ...
    expect(side_panel.locator('.deployment-header .title')).to_have_text(deployment_name)
...
def test_datatable_sort_functionality(page: Page):
    ...
    first_asc_value = page.locator('#deployments-table tbody tr.deployment-row:first-child').get_attribute('data-file-name')
    last_asc_value = page.locator('#deployments-table tbody tr.deployment-row:last-child').get_attribute('data-file-name')
    ...
    # Get first and last row data-file-name attributes
    first_desc_value = page.locator('#deployments-table tbody tr.deployment-row:first-child').get_attribute('data-file-name')
    last_desc_value = page.locator('#deployments-table tbody tr.deployment-row:last-child').get_attribute('data-file-name')

    # Assert that ascending order is opposite to descending order
    assert first_asc_value == last_desc_value, "first row sorted ascending should equal first row sorted descending"
    assert last_asc_value == first_desc_value, "last row sorted ascending should equal last row sorted descending"
...
def test_datatable_filter_application(page: Page):
    ...
    curator_filter_clear_button = curator_search_pane.locator('.dtsp-paneButton.clearButton')
    ...
    assert product_filter_count == '1', "After applying a filter, the product column filter trigger should show a count of 1"
    ...
    expect(filtered_rows).not_to_have_count(0)
    assert filtered_rows.count() == int(number_of_apple_options), "Number of filtered rows should match number of selected filter options"
...
def test_datatable_columns_and_headers(page: Page):
    ...
    assert data_col_idx is not None and data_col_idx.isdigit(), "'data-col-idx' attribute should exist and not be empty"
...
def test_datatable_error_handling(page: Page):
    ...
    serious_errors = [error for error in console_errors
                     if not any(harmless in error.lower() for harmless in [
                         'mathjax', 'failed to parse', 'warning'
                     ])]
